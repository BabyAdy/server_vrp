...
-- Sloturile vin din NUI ca numere; le fortam la int inainte sa ajunga pe retea,
-- ca sa nu existe niciodata desincronizare 0-based JS / 1-based Lua.
...
RegisterNUICallback('move', function(d, cb)
    TriggerServerEvent('ph_inventory:sv:move', num(d.from), num(d.to), num(d.count))
    cb('ok')
end)
RegisterNUICallback('loadAmmo', function(d, cb)
    TriggerServerEvent('ph_inventory:sv:loadAmmo', num(d.ammoSlot), num(d.weaponSlot))
    cb('ok')
end)
...
RegisterNUICallback('equip', function(d, cb)
    TriggerServerEvent('ph_inventory:sv:equip', num(d.slot), num(d.eqSlot))
    cb('ok')
end)
